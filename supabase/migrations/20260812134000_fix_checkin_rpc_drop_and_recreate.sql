/*
# PPSU Events — Fix check_in_ticket RPC parameter mismatch

PostgreSQL does not allow renaming an input parameter on an existing function with
CREATE OR REPLACE FUNCTION. We therefore drop the existing function by its exact
current signature and recreate it with the required p_input parameter while
preserving the original behavior and validation logic.

The only functional change is that ticket lookup accepts either the QR token or
an existing ticket_number.
*/

DROP FUNCTION IF EXISTS public.check_in_ticket(text, uuid);

CREATE OR REPLACE FUNCTION public.check_in_ticket(p_input text, p_staff_id uuid)
RETURNS TABLE (
  success boolean,
  ticket_number text,
  attendee_name text,
...
BEGIN
  ...
  FROM public.tickets
  WHERE qr_token = p_input
     OR ticket_number = p_input
  FOR UPDATE;
